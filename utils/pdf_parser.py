    ...
    try:
        with fitz.open(stream=data,filetype='pdf') as doc:
            for page in doc:
                ...
                def score(s):
                    low=s.lower(); keys=('certificate no','certificate number','unique id','product identification','item no','diameter','minimum breaking load','break load','description','lankhorst','bexco')
...
def _generic_component(extraction,text):
    ...
    minimum=extraction.get('minimum_breaking_load_kn'); ldbf=extraction.get('ldbf_kn')
    presentation='SPLICED' if re.search(r'spliced\s+(?:both\s+ends|one\s+end)|spliced\s*-\s*line',text,re.I) else ''
    onboard='LOOP_AROUND_BOLLARD' if ctype=='TAIL' else None
    applicable_label='LDBF' if ldbf is not None else ('Minimum breaking load' if minimum is not None else None)
    strength=component_from_certificate(component_id=str(cid),component_type=ctype,certificate_id=str(cert),minimum_breaking_load_kn=minimum,ldbf_kn=ldbf,final_presentation=presentation,onboard_application=onboard,applicable_load_label=applicable_label)
    loads={x.label:x.value_kn for x in strength.breaking_loads}
    return {'certificate_id':str(cert),'component_id':str(cid),'component_type':ctype,'item_description':str(product),'raw_material':'','final_presentation':presentation,
        'onboard_application':onboard,'applicable_break_load_label':strength.applicable_load_label,'diameter_mm':extraction.get('diameter_mm'),'length_m':extraction.get('length_m'),
        'break_load_linear_kn':loads.get('Break load linear'),'break_load_spliced_kn':loads.get('Break load spliced'),'break_load_grommet_kn':loads.get('Break load grommet'),
        'calculated_breaking_load_kn':extraction.get('calculated_breaking_load_kn'),'minimum_breaking_load_kn':minimum,'ldbf_kn':ldbf,'tdbf_kn':extraction.get('tdbf_kn'),'source_pages':(1,)},strength
...
def _to_legacy_dict(extraction,text):
    ...
    if not manufacturer:
        if re.search(r'\bBEXCO\b',text,re.I):manufacturer='Bexco'
        elif re.search(r'\bLANKHORST\s+ROPES?\b',text,re.I):manufacturer='Lankhorst Ropes'
