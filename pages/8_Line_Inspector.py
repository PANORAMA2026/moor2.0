...
# Preserve a line selected from the 3D Mooring Command Center.
requested_id=str(st.session_state.get("selected_line_id","")).strip()
requested_index=next((i for i,lbl in enumerate(labels) if lbl.split(" — ")[0].strip()==requested_id),0)
choice=st.selectbox("Seleziona linea",labels,index=requested_index,key="line_inspector_selector")
selected_id=str(inventory.iloc[labels.index(choice)][id_col]); st.session_state["selected_line_id"]=selected_id
...
st.caption("Ispezione fotografica assistita da Gemini, integrata nel record della linea. L'AI descrive solo ciò che è visibile; l'operatore deve confermare.")
