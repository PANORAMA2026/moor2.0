...
def init_ai_repository() -> None:
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS ai_line_inspections (
      ...
      model TEXT NOT NULL DEFAULT 'gemini-2.5-flash',provider TEXT NOT NULL DEFAULT 'Google Gemini',image_sha256 TEXT NOT NULL,
      image_mime TEXT NOT NULL,image_blob BLOB NOT NULL,image_quality TEXT NOT NULL DEFAULT 'UNDETERMINED',
      overall_severity TEXT NOT NULL DEFAULT 'UNDETERMINED',confidence REAL,findings_json TEXT NOT NULL DEFAULT '[]',ai_summary TEXT NOT NULL DEFAULT '',
...
def list_inspections(line_id: str|None=None)->list[dict[str,Any]]:
    ...
    try:
        select=",".join(cols)
        if line_id: rows=conn.execute(f"SELECT {select} FROM ai_line_inspections WHERE line_id=? ORDER BY timestamp_utc DESC",(str(line_id),)).fetchall()
        else: rows=conn.execute(f"SELECT {select} FROM ai_line_inspections ORDER BY timestamp_utc DESC").fetchall()
    finally: conn.close()
