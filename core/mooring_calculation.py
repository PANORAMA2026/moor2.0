...
def run_mooring_calculation(
    ...
    *,
    ...
    pretension_pct: float | None = None,
) -> tuple[pd.DataFrame, EnvironmentalLoadResult]:
    """Run the complete engineering chain.

    If ``pretension_pct`` is ``None``, the solver uses the per-line
    ``pretension_pct`` column supplied by the active mooring setup.
    """
    heave = 0.0
    if environmental_state.water_level_m is not None:
        heave = vessel_heave_from_tide(
            ...
            tide_response_factor,
        )
        geom_df = apply_vessel_heave_to_geometry(geom_df, heave)

    loads = calculate_environmental_loads(
        environmental_state,
        vessel,
    ...
    results = solve_line_tensions_3d(
        geom_df,
        loads.as_legacy_solver_dict(),
