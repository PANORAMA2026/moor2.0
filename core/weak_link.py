"""Engineering capacity selection after complete mooring-component identification."""
...
def infer_applicable_load_label(*, component_type: str, final_presentation: str = "", onboard_application: str | None = None,
                                has_ldbf: bool = False, has_minimum_breaking_load: bool = False,
                                has_spliced: bool = False, has_linear: bool = False, has_grommet: bool = False) -> str | None:
    ...
    if "loop_around_bollard" in application or "loop around bollard" in application:
        return "Break load grommet" if has_grommet else None
    if has_ldbf:
        return "LDBF"
    if "splice" in presentation and has_spliced:
        return "Break load spliced"
    if "linear" in presentation and has_linear:
        return "Break load linear"
    if has_spliced and ("spliced" in presentation or not has_linear):
        return "Break load spliced"
    if has_linear:
        return "Break load linear"
    if has_minimum_breaking_load:
        return "Minimum breaking load"
    return None
...
def evaluate_weak_link(components: Iterable[ComponentStrength | dict]) -> WeakLinkResult:
    ...
    if missing:
        return WeakLinkResult("INCOMPLETE", None, None, None, None, None, None, normalized,
            "Applicable breaking-load data are missing for component(s): " + ", ".join(missing) + ". Complete the component records before assigning an assembly weak link.")
    if len(normalized) == 1:
        c = normalized[0]; value = c.governing_breaking_load_kn
        return WeakLinkResult("NO_WEAK_LINK", None, None, c.certificate_id, value,
            value / 9.80665 if value is not None else None, c.governing_breaking_load_label, normalized,
    ...
    return WeakLinkResult("VALID", weak.component_id, weak.component_type, weak.certificate_id, value_kn,
        value_kn / 9.80665 if value_kn is not None else None, weak.governing_breaking_load_label, normalized,
...
def component_from_certificate(*, component_id: str, component_type: str, certificate_id: str | None,
                                ...
                                final_presentation: str = "", onboard_application: str | None = None,
                                applicable_load_label: str | None = None) -> ComponentStrength:
    values = []
    for value, label in ((break_load_linear_kn, "Break load linear"), (break_load_spliced_kn, "Break load spliced"),
                         (break_load_grommet_kn, "Break load grommet"), (minimum_breaking_load_kn, "Minimum breaking load"),
                         (ldbf_kn, "LDBF")):
        if value is not None and float(value) > 0:
            values.append(BreakingLoadValue(float(value), label))
    selected = applicable_load_label or infer_applicable_load_label(
        component_type=component_type, final_presentation=final_presentation, onboard_application=onboard_application,
        has_ldbf=ldbf_kn is not None, has_minimum_breaking_load=minimum_breaking_load_kn is not None,
        has_spliced=break_load_spliced_kn is not None, has_linear=break_load_linear_kn is not None,
        has_grommet=break_load_grommet_kn is not None)
    return ComponentStrength(component_id=component_id, component_type=component_type, certificate_id=certificate_id,
