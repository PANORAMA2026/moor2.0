...
def get_material_stiffness(material_type: str, tension_tons: float, mbl_tons: float, length_m: float) -> float:
    ...
    strain = {"HMPE": 0.025, "POLYESTER": 0.070, "NYLON": 0.180, "STEEL_WIRE": 0.012}.get(
        _material_key(material_type), 0.050
    )
...
def calculate_composite_stiffness(
    ...
    if tail_len > 0:
        if tail_mbl is None or float(tail_mbl) <= 0:
    ...
    if geolink_len > 0:
        if geolink_mbl is None or float(geolink_mbl) <= 0:
            raise ValueError("GeoLink length is positive but GeoLink break strength is invalid.")
...
def solve_line_tensions_3d(
    ...
    for iteration in range(1, max_iter + 1):
        ...
        for idx, row in df.iterrows():
            ...
            geo_len = float(row.get("geolink_length_m", 0.0) or 0.0)
            geo_mbl = row.get("geolink_mbl_tons", None)
            ...
            if geo_len > 0 and (geo_mbl is None or pd.isna(geo_mbl) or float(geo_mbl) <= 0):
                raise ValueError(f"Line {row.get('line_id', idx)} has a GeoLink length but no valid GeoLink break strength.")
            main_mbl = float(row.get("main_mbl_tons", row["mbl_tons"]) or row["mbl_tons"])
            if main_mbl <= 0:
                raise ValueError(f"Line {row.get('line_id', idx)} has no valid main-line breaking strength.")
            k_eq = calculate_composite_stiffness(
                ...
                tail_len,
                str(row.get("geolink_material", "UNKNOWN")),
                None if geo_mbl is None or pd.isna(geo_mbl) else float(geo_mbl),
                geo_len,
            )
            k_list.append(k_eq)
        ...
        updated = np.maximum(
            0.0,
            np.array([base_pretension[i] + k_list[i] * float(np.dot(b_vectors[i], displacement)) for i in range(len(df))]),
        )
        resisting = np.sum([updated[i] * b_vectors[i] for i in range(len(updated))], axis=0)
    ...
    df.attrs["solver_diagnostics"] = SolverDiagnostics(
        ...
        iterations,
        residual_norm,
        "Equilibrium converged." if status == SolverStatus.CONVERGED else "Solver did not produce a verified equilibrium.",
    )
