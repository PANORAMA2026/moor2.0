...
The transform is explicit: corresponding drawing points and known ship X/Y
coordinates are supplied by the operator. No scale or offset is guessed.
"""
...
def fit_affine(points_px: Sequence[Sequence[float]], points_xy_m: Sequence[Sequence[float]]):
    ...
    pred=A@coef
    rms=float(np.sqrt(np.mean(np.sum((pred-target)**2,axis=1))))
    return coef,rms


def apply_affine(px: float, py: float, coef):
    out=np.array([float(px),float(py),1.0])@np.asarray(coef)
    return float(out[0]),float(out[1])


def save_calibration(station_name: str, drawing_width_px: float, drawing_height_px: float, coef, rms_error_m: float):
...
def load_calibration(station_name: str):
    conn=sqlite3.connect(DB_FILE_PATH,check_same_thread=False)
    try: row=conn.execute("SELECT * FROM mooring_plan_calibration WHERE station_name=?",(station_name,)).fetchone()
