...
The parser is intentionally layered: native PDF text first, table extraction
second, and optional OCR integration point third. Extraction never makes a
value certified; every field retains provenance and confidence.
"""
...
FIELD_PATTERNS = {
    "ship_design_mbl": [
        r"ship\s+design\s+mbl\s*[:=]?\s*([\d.,]+)\s*(kn|t|tonnes?|tons?)?",
    ],
    "ldbf": [
        r"(?:line\s+design\s+break\s+force|ldbf)\s*[:=]?\s*([\d.,]+)\s*(kn|t|tonnes?|tons?)?",
    ],
    ...
    "tail_design_break_force": [
        r"(?:tail\s+design\s+break\s+force|tdbf)\s*[:=]?\s*([\d.,]+)\s*(kn|t|tonnes?|tons?)?",
    ],
...
def _number(s: str) -> float:
    # Certificate numbers are commonly formatted as 1,234.56. Do not treat
    # commas as decimal separators because engineering source formats vary.
...
def _extract_from_text(text: str, page: int | None = None, certificate_type: str = "MOORING_LINE") -> CertificateExtraction:
    ...
    # MEG4 certificate templates commonly identify immediate strain by the
    # load percentage. Support both '% LDBF ... value' and '10% ... value'.
    strain_patterns = [
    ...
    for pattern in strain_patterns:
        for m in pattern.finditer(normalized):
            ...
            result.fields.append(ExtractedField(
                f"average_immediate_strain_{pct}_pct_{basis}",
                _number(m.group(2)), m.group(0), 0.88, page, "%",
            ))
    return result


def parse_certificate_text(text: str, certificate_type: str = "MOORING_LINE") -> CertificateExtraction:
    """Extract fields from already-extracted text, conservatively."""
...
def extract_pdf_text(pdf_path: str | Path) -> tuple[str, int, int, str, list[str]]:
    """Extract text using PyMuPDF, with pypdf fallback.

    Returns text, pages_with_text, total_pages, method, warnings. Image-only
    PDFs are detected explicitly; OCR is not silently attempted.
    """
    path = Path(pdf_path)
    if not path.exists():
...
def parse_certificate_pdf(pdf_path: str | Path, certificate_type: str = "AUTO") -> CertificateExtraction:
    """Parse a PDF and retain page/provenance information for each match."""
    ...
    # Parse page-by-page to preserve source page numbers.
    offset = 0
    ...
    # Duplicate fields across pages are ambiguous unless values agree.
    ...
    for name, candidates in by_name.items():
        ...
        if len(candidates) == 1 or len(values) == 1:
            result.fields.append(candidates[0])
        else:
...
def validate_extraction(extraction: CertificateExtraction) -> list[str]:
    ...
    for name in ("ldbf", "tail_design_break_force", "diameter_mm", "length_m"):
