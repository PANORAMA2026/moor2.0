...
class CertificateExtraction:
    certificate_type:str="UNKNOWN"; fields:list[ExtractedField]=field(default_factory=list); warnings:list[str]=field(default_factory=list); raw_text:str=""; pages_with_text:int=0; total_pages:int=0; extraction_method:str="none"
...
def parse_certificate_text(text: str,certificate_type: str="AUTO"):
    text=text or ""; certificate_type="MOORING_TAIL" if certificate_type=="AUTO" and re.search(r"\b(?:tail|tdbf)\b",text,re.I) else ("MOORING_LINE" if certificate_type=="AUTO" else certificate_type)
    ...
    aliases={
      ...
      "length_m":[r"length\s*\(?(?:m|mtr|metres?)\)?\s*[:=\-]?\s*\d+\s*[x×]\s*(\d+(?:[.,]\d+)?)\s*m\b",r"length\s*\(?(?:m|mtr|metres?)\)?\s*[:=\-]?\s*(\d+(?:[.,]\d+)?)\s*m\b",r"quantity\s*[:=\-]?\s*\d+\s*[x×]\s*(\d+(?:[.,]\d+)?)\s*m\b",r"\bCL\s*(\d+(?:[.,]\d+)?)\s*MTR\b"],
    ...
    # Detect conflicting explicit LDBF declarations instead of silently taking the first one.
    ldbf_matches=[_num(m.group(1)) for m in re.finditer(r"\bLDBF\b[^\n]{0,35}?([\d.,]+)\s*kN\b",text,re.I)]
    ...
    for m in re.finditer(r"(?:average\s+)?(?:immediate\s+)?strain[^\n]*?at\s*(\d+)\s*%\s*(?:ldbf)?[^\n]*?([\d.,]+)\s*%",text,re.I):_add(r,f"average_immediate_strain_{m.group(1)}_pct_ldbf",_num(m.group(2)),m.group(0),"%")
    unique={}
    for f in r.fields:
...
def validate_extraction(extraction):return [] if (extraction.get("ldbf") is not None or extraction.get("minimum_breaking_load") is not None or extraction.get("tail_design_break_force") is not None) else ["No declared breaking-load value extracted"]
