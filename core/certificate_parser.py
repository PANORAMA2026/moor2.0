...
def parse_certificate_text(text: str, certificate_type: str="AUTO") -> CertificateExtraction:
    ...
    # Legacy MEG-style strain table, retained as separate fields.
    ...
    # Lankhorst combined LDBF/TDBF cell.
...
def validate_extraction(extraction: CertificateExtraction) -> list[str]:
    errors=[]
    if extraction.get("ldbf") is not None or extraction.get("minimum_breaking_load") is not None or extraction.get("minimum_breaking_load_kn") is not None:
        pass
    elif extraction.get("tail_design_break_force") is not None: pass
    else: errors.append("No declared breaking-load value extracted")
    return errors
