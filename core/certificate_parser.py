...
def _extract_from_text(text: str, page: int | None = None, certificate_type: str = "MOORING_LINE") -> CertificateExtraction:
    ...
    for name, patterns in FIELD_PATTERNS.items():
        ...
        if len(matches) == 1:
            ...
            result.fields.append(ExtractedField(name, _number(m.group(1)), m.group(0), 0.95, page, unit))
        elif len(matches) > 1:
...
def parse_certificate_pdf(pdf_path: str | Path, certificate_type: str = "AUTO") -> CertificateExtraction:
    ...
    if not text.strip():
        result.warnings.append("No machine-readable text found. OCR/manual review required.")
        return result
    try:
        import fitz
...
def validate_extraction(extraction: CertificateExtraction) -> list[str]:
    errors: list[str] = []
    # Manufacturer certificates can provide MBL without stating LDBF. Never
    # infer LDBF from a calculated breaking load unless the source labels it LDBF.
    required = {"minimum_breaking_load", "diameter_mm", "length_m"} if extraction.certificate_type != "MOORING_TAIL" else {"tail_design_break_force"}
