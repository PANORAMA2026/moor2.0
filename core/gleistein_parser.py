...
class GleisteinComponent:
    ...
    def strength(self):return component_from_certificate(component_id=self.component_id,component_type=self.component_type,certificate_id=self.certificate_id,break_load_linear_kn=self.break_load_linear_kn,break_load_spliced_kn=self.break_load_spliced_kn,break_load_grommet_kn=self.break_load_grommet_kn)
def _num(s):
    s=str(s).replace(' ','')
...
def _normalize_cert_id(s):
    s=_clean(s).upper(); s=re.sub(r'^W(?:2Z25|Z25|225)-','W225-',s); return s
...
def _groups(pages):
    ...
    for n,text in enumerate(pages,1):
        item_m=re.search(r'Item\s+No\.\s+client\s*/\s*Gleistein\s*:\s*(.+)',text,re.I); cert_m=re.search(r'Certificate\s+no\.\s*:\s*([A-Z0-9-]+)',text,re.I)
        if item_m:
            current={'pages':[(n,text)],'item':_clean(item_m.group(1)),'cert_id':_normalize_cert_id(cert_m.group(1) if cert_m else '')}; groups.append(current)
        elif current is not None:current['pages'].append((n,text))
    return groups
def parse_gleistein_pages(page_texts):
    pages=list(page_texts or []); components=[]; warnings=[]
    for group in _groups(pages):
        text='\n'.join(t for _,t in group['pages']); item=group['item']; cert_id=group['cert_id']; parts=re.split(r'\s*/\s*',item,maxsplit=1); comp_id=_clean(parts[1]) if len(parts)==2 else cert_id
