...
class GleisteinComponent:
    certificate_id:str; component_id:str; component_type:str; item_description:str=""; raw_material:str=""; final_presentation:str=""; onboard_application:str|None=None; applicable_break_load_label:str|None=None; diameter_mm:float|None=None; length_m:float|None=None; break_load_linear_kn:float|None=None; break_load_spliced_kn:float|None=None; break_load_grommet_kn:float|None=None; calculated_breaking_load_kn:float|None=None; source_pages:tuple[int,...]=()
    def strength(self):return component_from_certificate(component_id=self.component_id,component_type=self.component_type,certificate_id=self.certificate_id,break_load_linear_kn=self.break_load_linear_kn,break_load_spliced_kn=self.break_load_spliced_kn,break_load_grommet_kn=self.break_load_grommet_kn,final_presentation=self.final_presentation,onboard_application=self.onboard_application,applicable_load_label=self.applicable_break_load_label)
...
def _normalize_cert_id(s):return re.sub(r"^W(?:2Z25|WZ25|Z25|225)-","W225-",_clean(s).upper())
...
def _value(text,label):
    ...
    probe=" ".join(x.strip() for x in text[m.end():].splitlines()[:3] if x.strip());n=re.search(r"[-+]?\d[\d\s]*(?:[.,]\d[\d\s.,]*)?",probe)
...
def _item_from_text(text):
    # Highest confidence: the actual item code line. OCR may separate the
    # label and value or reorder columns, so do not depend on one layout.
    m=re.search(r"\b(MAIN\s+LINE|TAIL|LASHING|GEOLINK)\s*/\s*([A-Z0-9_-]+)",text,re.I)
    if m:return f"{_clean(m.group(1))} / {_clean(m.group(2))}"
    m=re.search(r"Item\s+No\.\s+client\s*/\s*Gleistein\s*:?[ \t]*([^\n]*)",text,re.I)
    if not m:return ""
    value=_clean(m.group(1))
    if value.lower() in {"item description","description"} or not value:
        # Search only the next few lines for another structured item token.
        for line in text[m.end():].splitlines()[:8]:
            mm=re.search(r"\b(MAIN\s+LINE|TAIL|LASHING|GEOLINK)\s*/\s*([A-Z0-9_-]+)",line,re.I)
            if mm:return f"{_clean(mm.group(1))} / {_clean(mm.group(2))}"
    return value

def _component_id(text,ctype,item,cert_id):
    if "/" in item:
        value=item.split("/",1)[1].strip()
        if value and not value.startswith("G236"):return _clean(value)
...
def parse_gleistein_pages(page_texts):
    ...
    for group in _groups(pages):
        ...
        components.append(GleisteinComponent(certificate_id=cert,component_id=cid,component_type=ctype,item_description=desc,raw_material=_field(text,"Raw material"),final_presentation=presentation,onboard_application=onboard,applicable_break_load_label=probe.applicable_load_label,diameter_mm=_diameter(text),length_m=_length(text),break_load_linear_kn=linear,break_load_spliced_kn=spliced,break_load_grommet_kn=grommet,calculated_breaking_load_kn=calculated,source_pages=tuple(n for n,_ in group["pages"])))
