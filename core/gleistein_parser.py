...
class GleisteinComponent:
    ...
    def strength(self):
        return component_from_certificate(component_id=self.component_id,component_type=self.component_type,certificate_id=self.certificate_id,
            break_load_linear_kn=self.break_load_linear_kn,break_load_spliced_kn=self.break_load_spliced_kn,break_load_grommet_kn=self.break_load_grommet_kn,
            final_presentation=self.final_presentation,onboard_application=self.onboard_application,applicable_load_label=self.applicable_break_load_label)
...
def _component_id(text,ctype,item,cert_id):
    # Explicit item number is the highest-confidence physical component ID.
    if "/" in item:
        value=item.split("/",1)[1].strip()
        if value and value.lower() not in {"item description", "description"}:return _clean(value)
    patterns={
      "TAIL":[r"\b\d{6,}[xX]\d{1,3}\b"],
      "GEOLINK/LASHING":[r"(?:LASHING|GEOLINK)[^\n]*?\b(\d{8,})\b"],
      "MAIN LINE":[r"\b6FT[A-Z0-9_-]+\b"],
    }
...
def _groups(pages):
    # A certificate number is useful, but OCR often omits it from continuation
    # pages. Item No. client / Gleistein is therefore the primary group boundary.
    groups=[];current=None
    for n,text in enumerate(pages,1):
        item=_item_from_text(text);cert=_cert_id_from_text(text)
        if item and current is not None:
            current={"pages":[],"item":item,"cert_id":cert};groups.append(current)
...
def parse_gleistein_pages(page_texts):
    ...
    for group in _groups(pages):
        ...
        desc_m=re.search(r"Item\s+description\s*:?\s*(.+?)(?=\n(?:Final\s+presentation|Raw\s+material|Delivered\s+quantity|Break\s+load|Weight))",text,re.I|re.S)
        desc=_clean(desc_m.group(1)) if desc_m else ""
        ctype=_classify(text,item,desc);cid=_component_id(text,ctype,item,cert)
        presentation=_field(text,"Final presentation");onboard=ONBOARD_TAIL_APPLICATION if ctype=="TAIL" else None
