...
def init_mooring_geometry_db():
    ...
    station_name TEXT NOT NULL,line_id TEXT NOT NULL,winch_id TEXT,fairlead_id TEXT,bollard_id TEXT,
...
def seed_station_from_aft_catalog(station_name="Poppa (Aft Station)"):
    """Import source-identified AFT equipment without inventing coordinates."""
...
def upsert_connection(station_name:str,line_id:str,winch_id:str,fairlead_id:str,bollard_id:str,rope_diameter_mm:Optional[float]=None,route_nodes:Optional[Sequence[str]]=None):
    init_mooring_geometry_db(); conn=_conn(); conn.execute("""INSERT INTO mooring_connections(station_name,line_id,winch_id,fairlead_id,bollard_id,rope_diameter_mm) VALUES(?,?,?,?,?,?) ON CONFLICT(station_name,line_id) DO UPDATE SET winch_id=excluded.winch_id,fairlead_id=excluded.fairlead_id,bollard_id=excluded.bollard_id,rope_diameter_mm=excluded.rope_diameter_mm""",(station_name,line_id,winch_id,fairlead_id,bollard_id,rope_diameter_mm)); conn.execute("DELETE FROM mooring_route_nodes WHERE station_name=? AND line_id=?",(station_name,line_id)); route=list(route_nodes) if route_nodes else [winch_id,fairlead_id,bollard_id]
...
def route_geometry(station_name,line_id):
    comps=get_components(station_name); nodes=get_route_nodes(station_name,line_id)
    if not nodes:
    ...
    if len(points)==len(nodes):
        segments=[math.dist(a,b) for (_,a),(_,b) in zip(points,points[1:])]; length=sum(segments)
    changes=[]
    if len(points)==len(nodes):
        for i in range(1,len(points)-1):
            a,c,b=points[i-1][1],points[i][1],points[i+1][1]
...
def recalculate_connection_geometry(station_name,line_id):
    rows=get_connections(station_name); rows=rows[rows.line_id.astype(str)==str(line_id)]
    if rows.empty:return
    r=rows.iloc[0]; nodes=get_route_nodes(station_name,line_id) or [str(r.get("winch_id")),str(r.get("fairlead_id")),str(r.get("bollard_id"))]; comps=get_components(station_name); points=[_point(comps,cid) for cid in nodes]
    ...
    if all(p is not None for p in points):
        ...
        if len(points)>=3: c=points[1]; angle=_angle_between(tuple(points[0][i]-c[i] for i in range(3)),tuple(points[2][i]-c[i] for i in range(3)))
...
def get_line_detail(station_name,line_id):
    ...
    for key,label in [("winch_id","Winch"),("fairlead_id","Fairlead"),("bollard_id","Bollard")]:
        ...
        if cid:
            c=comps[comps.component_id.astype(str)==str(cid)]
            if not c.empty:
