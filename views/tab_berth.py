...
def _profile_dataframe(selected_port: str) -> tuple[pd.DataFrame, float | None]:
    ...
    rows = [{
        "bollard_id": str(point.bollard_id), "measurement_station": str(point.measurement_station),
        "side": str(point.side), "x_m": float(point.x_m), "y_m": float(point.y_m),
        "z_m": float(point.z_m), "survey_water_level_m": float(point.survey_water_level_m)
...
def _add_ship(fig: go.Figure, ship: dict, offset: float) -> tuple[float, float, float]:
    ...
    wing_y = beam_max / 2.0
    fig.add_trace(go.Scatter3d(x=[bridge_x, bridge_x], y=[-wing_y, wing_y], z=[bridge_eye, bridge_eye], mode="lines+markers", line=dict(width=5), marker=dict(size=3), name="Bridge Wings (reference)", hoverinfo="skip"))
...
def _add_platform_origins(fig: go.Figure, profile_name: str, ship_offset: float) -> None:
    ...
    for station in ("FWD", "AFT"):
        ...
        if not p:
            continue
        x, y, z = float(p["x_m"]) + float(ship_offset), float(p["y_m"]), float(p["z_m"])
        deck = "Deck 3" if station == "FWD" else "Deck 1"
        note = "27 m aft of extreme bow" if station == "FWD" else "14 m forward of extreme stern"
        fig.add_trace(go.Scatter3d(x=[x], y=[y], z=[z], mode="markers+text", text=[f"{station} MOORING PLATFORM — {deck}"], textposition="top center", marker=dict(size=8, symbol="cross"), name=f"{station} mooring platform", showlegend=False, hovertemplate=f"{station} mooring platform — {deck}<br>{note}<br>X=%{{x:.2f}} m<br>Y=%{{y:.2f}} m<br>Z=%{{z:.2f}} m<extra></extra>"))
...
def _add_fixed_berth(fig: go.Figure, bollards: pd.DataFrame) -> None:
    ...
    x, y, z = [pd.to_numeric(bollards[c], errors="coerce") for c in ("x_m", "y_m", "z_m")]
...
def _add_berth_block(fig: go.Figure, bollards: pd.DataFrame, profile_name: str) -> None:
    ...
    fig.add_trace(go.Mesh3d(x=xs, y=ys, z=zs, i=i, j=j, k=k, color="#3A8F5B", opacity=0.58, flatshading=False, lighting=dict(ambient=0.35, diffuse=0.80, specular=0.12, roughness=0.85), lightposition=dict(x=100, y=-100, z=200), name=f"{profile_name} — 3D berth solid", hovertemplate="Berth 3D solid<br>Top reference=%{z:.2f} m<br>Depth=%{customdata:.1f} m<extra></extra>", customdata=[BERTH_BLOCK_DEPTH_M] * 8))
...
def _calibration_from_session(station: str) -> PlanCalibration:
    ...
    if not values:
        return default
    return PlanCalibration(station=station, anchor_u_px=float(values["u"]), anchor_v_px=float(values["v"]), anchor_x_m=default.anchor_x_m, anchor_y_m=default.anchor_y_m, anchor_z_m=default.anchor_z_m, scale_x_m_per_px=float(values["sx"]), scale_y_m_per_px=float(values["sy"]), rotation_deg=float(values["rot"]))
...
def _render_reference_calibration_controls() -> tuple[bool, float, dict[str, PlanCalibration]]:
    with st.expander("🧭 Modalità B — Pianetti 2D sovrapposti al modello 3D", expanded=True):
        enabled = st.checkbox("Mostra i pianetti 2D direttamente nella scena 3D", value=True, help="Reference layer: non modifica ancora le coordinate del solver.")
        opacity = st.slider("Opacità pianetti", 0.05, 0.70, 0.24, 0.01)
        st.caption("Clicca direttamente sul centro della mooring platform nel pianetto. Il click imposta automaticamente l'anchor U/V. +X = PRUA, +Y = PORT.")
        cols = st.columns(2)
        for col, station in zip(cols, ("FWD", "AFT")):
            default = get_default_calibration(station)
            key = f"plan_cal_{station}"
            if key not in st.session_state:
                st.session_state[key] = {"u": default.anchor_u_px, "v": default.anchor_v_px, "sx": default.scale_x_m_per_px, "sy": default.scale_y_m_per_px, "rot": default.rotation_deg}
            with col:
                st.markdown(f"**{station} — Deck {'3' if station == 'FWD' else '1'}**")
                click = streamlit_image_coordinates(get_mooring_plan_image(station), key=f"{station}_plan_click")
                if click is not None and "x" in click and "y" in click:
                    st.session_state[key]["u"] = float(click["x"])
                    st.session_state[key]["v"] = float(click["y"])
                    st.session_state[f"{station}_u"] = float(click["x"])
                    st.session_state[f"{station}_v"] = float(click["y"])
                    st.success(f"Anchor {station}: U={click['x']:.0f}, V={click['y']:.0f} px")
                ...
                st.session_state[key] = {"u": st.session_state[f"{station}_u"], "v": st.session_state[f"{station}_v"], "sx": st.session_state[f"{station}_sx"], "sy": st.session_state[f"{station}_sy"], "rot": st.session_state[f"{station}_rot"]}
                if st.button(f"Ripristina calibrazione {station}", key=f"reset_{station}"):
                    st.session_state[key] = {"u": default.anchor_u_px, "v": default.anchor_v_px, "sx": default.scale_x_m_per_px, "sy": default.scale_y_m_per_px, "rot": default.rotation_deg}
                    st.rerun()
        st.info("Calibrazione iniziale = VISIVA/PRELIMINARE. Il click sulla piattaforma serve a portare il disegno nella stessa origine 3D della nave. Le coordinate dei singoli elementi saranno promosse a geometria engineering solo dopo validazione.")
...
def _add_plan_overlay(fig: go.Figure, station: str, calibration: PlanCalibration, opacity: float, ship_offset: float) -> None:
    ...
    max_dim = 110
    resize_factor = min(1.0, max_dim / max(image.size))
    small = image.resize((max(2, int(image.width * resize_factor)), max(2, int(image.height * resize_factor))))
    arr = np.asarray(small, dtype=float) / 255.0
    h, w = arr.shape
    px_scale_u = w / image.width
    px_scale_v = h / image.height
    u = np.arange(w) / px_scale_u
    v = np.arange(h) / px_scale_v
    U, V = np.meshgrid(u, v)
    ...
    fig.add_trace(go.Surface(x=X, y=Y, z=Z, surfacecolor=arr, colorscale=[[0.0, "black"], [1.0, "white"]], cmin=0.0, cmax=1.0, showscale=False, opacity=opacity, name=f"{station} mooring plan — reference overlay", hoverinfo="skip", showlegend=True))
...
def _figure_3d(ship: dict, bollards: pd.DataFrame, offset: float, show_plans: bool, plan_opacity: float, calibrations: dict[str, PlanCalibration]) -> tuple[go.Figure, tuple[float, float, float]]:
    ...
    fig.update_layout(height=800, margin=dict(l=0, r=0, t=25, b=0), scene=dict(xaxis_title="X — Longitudinale (+ PRUA) [m]", yaxis_title="Y — Trasversale (+ PORT) [m]", zaxis_title="Z — Quota [m]", aspectmode="data", camera=dict(eye=dict(x=1.65, y=1.55, z=1.05))), legend=dict(orientation="h", yanchor="bottom", y=1.01, x=0))
...
def render_tab_berth(selected_port, ship_dict):
    ...
    if is_real_profile:
        ...
        st.caption(f"Riferimento rilievo: livello acqua +{survey_level:.2f} m. FWD: origine rilievo su Deck 3, 27 m a poppavia dell'estrema prua. AFT: origine rilievo su Deck 1, 14 m a pruavia dell'estrema poppa. Le bitte sono fisse; la nave e le mooring platforms traslano soltanto longitudinalmente.")
    else:
    ...
    if is_real_profile:
        ...
        try:
            ...
            st.caption("Modello calibrato sul profilo Carnival Panorama: LOA 323.44 m, Beam 37.20 m, Draft 8.5 m. I pianetti FWD/AFT sono reference overlay calibrati rispetto alle mooring platforms; non sono ancora sorgente automatica di coordinate del solver.")
