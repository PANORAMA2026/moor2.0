...
def _add_ship(fig: go.Figure, ship: dict, offset: float):
...
def _add_platforms(fig: go.Figure, ship_offset: float) -> None:
    for station, p in get_mooring_platforms().items():
        ...
        fig.add_trace(go.Scatter3d(
            ...
            marker=dict(size=9, symbol="cross"), name=f"{station} mooring platform", showlegend=False,
...
def _add_fairleads(fig: go.Figure, ship_offset: float, station_filter: str = "ALL") -> pd.DataFrame:
    ...
    fig.add_trace(go.Scatter3d(
        ...
        marker=dict(size=7, symbol="circle"), name="Fairleads — PORT", customdata=custom,
        hovertemplate=("%{text}<br>Station=%{customdata[0]} — Deck %{customdata[1]}<br>"
                       "Frame=%{customdata[2]}<br>X=%{x:.2f} m<br>Y=%{y:.2f} m<br>"
                       "Z=%{z:.2f} m<br>Geometry=%{customdata[3]}<extra></extra>"),
    ))
    return df
...
def _add_berth_block(fig: go.Figure, bollards: pd.DataFrame, profile_name: str) -> None:
    ...
    fig.add_trace(go.Mesh3d(
        x=xs, y=ys, z=zs, i=i, j=j, k=k, color="#3A8F5B", opacity=0.58,
        flatshading=False, lighting=dict(ambient=0.35, diffuse=0.80, specular=0.12, roughness=0.85),
        lightposition=dict(x=100, y=-100, z=200), name=f"{profile_name} — 3D berth solid",
...
def _add_fixed_berth(fig: go.Figure, bollards: pd.DataFrame) -> None:
    ...
    fig.add_trace(go.Scatter3d(
        x=x.tolist(), y=y.tolist(), z=z.tolist(), mode="markers+text",
        text=bollards["bollard_id"].astype(str).tolist(), textposition="top center", customdata=labels,
        marker=dict(size=9, symbol="diamond"), name="Ensenada Pier #2 — PORT Bollards",
        hovertemplate="%{customdata}<br>X=%{x:.2f} m<br>Y=%{y:.2f} m<br>Z=%{z:.2f} m<extra></extra>",
    ))


def _connection_dataframe(bollards: pd.DataFrame, ship_offset: float):
    ...
    for conn in get_normal_setup():
        ...
        records.append({"line_id": conn.line_id, "station": conn.station, "line_type": conn.line_type,
                        "fairlead_id": conn.fairlead_id, "bollard_id": conn.bollard_id,
                        "fairlead_x_m": fx, "fairlead_y_m": fy, "fairlead_z_m": fz,
                        "bollard_x_m": bx, "bollard_y_m": by, "bollard_z_m": bz,
                        "straight_3d_length_m": length, "status": conn.status})
...
def _add_mooring_connections(fig: go.Figure, connections: pd.DataFrame) -> None:
    ...
    for _, row in connections.iterrows():
        ...
        shown.add(kind)
        fig.add_trace(go.Scatter3d(
            x=[row["fairlead_x_m"], row["bollard_x_m"]], y=[row["fairlead_y_m"], row["bollard_y_m"]],
            z=[row["fairlead_z_m"], row["bollard_z_m"]], mode="lines", line=style,
            name=f"{kind} lines" if showlegend else kind, legendgroup=kind, showlegend=showlegend,
            hovertemplate=(f"{row['line_id']} — {kind}<br>Fairlead: {row['fairlead_id']}<br>"
...
def _figure_3d(ship: dict, bollards: pd.DataFrame, offset: float, fairlead_station: str):
    fig = go.Figure()
    ...
    fig.update_layout(
        height=820, margin=dict(l=0, r=0, t=25, b=0),
        scene=dict(xaxis_title="X — Longitudinale (+ PRUA) [m]", yaxis_title="Y — Trasversale (+ PORT) [m]",
                   zaxis_title="Z — Quota [m]", aspectmode="data",
                   camera=dict(eye=dict(x=1.65, y=1.55, z=1.05))),
        legend=dict(orientation="h", yanchor="bottom", y=1.01, x=0),
    )
    return fig, model_dims, fairlead_df, unresolved
...
def render_tab_berth(selected_port, ship_dict):
    ...
    st.caption(f"Rilievo a livello acqua +{survey_level:.2f} m. FWD platform: Deck 3, 27 m a poppavia dell'estrema prua. AFT platform: Deck 1, 14 m a pruavia dell'estrema poppa. Le bitte sono fisse; la nave e gli elementi di bordo traslano longitudinalmente.")
    ...
    try:
        fig, model_dims, fairlead_df, unresolved = _figure_3d(ship_dict, df, float(new_offset), station)
    ...
    st.caption("Le 18 connessioni sono la topologia del normal setup Ensenada fornita dall'equipaggio. I fairlead sono ancora geometria REFERENCE derivata dal disegno; le linee mostrano la connessione geometrica attuale e la loro lunghezza rettilinea 3D, non una lunghezza reale sotto sag/pretensionamento.")
    ...
    st.info("Passo successivo: validare visivamente i fairlead/chock del disegno e poi sostituire i collegamenti REFERENCE con i punti engineering-grade. Dopo questa validazione potremo aggiungere winch → fairlead → linea → bitta e rendere ogni collegamento modificabile.")
