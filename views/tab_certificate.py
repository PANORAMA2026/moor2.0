...
def _component_rows(cdata):
    ...
    for c in cdata.get("components", []):
        rows.append({
            "Component": c.get("component_type"), "Component ID": c.get("component_id"),
            "Certificate": c.get("certificate_id"), "Diameter mm": c.get("diameter_mm"),
            "Length m": c.get("length_m"), "Linear kN": c.get("break_load_linear_kn"),
            "Spliced kN": c.get("break_load_spliced_kn"), "Grommet kN": c.get("break_load_grommet_kn"),
            "Applicable": c.get("applicable_break_load_label"), "Applicable kN": _applicable_kn(c),
...
def render_tab_certificate():
    ...
    st.caption("PDF → OCR → tutti i componenti → limiti dichiarati → applicazione di bordo → weak link solo se l'insieme è composto da più elementi")
    ...
    with right:
        ...
        if weak.get("status") == "VALID" and weak.get("has_weak_link"):
            st.metric("🔗 ASSEMBLY WEAK LINK", f"{weak_t:.2f} t")
            st.caption(f"Elemento che governa l'assemblaggio dopo l'identificazione di tutti i componenti: {weak.get('weak_link_component_type')} / {weak.get('weak_link_component_id')} — {weak.get('weak_link_value_label')}")
        elif weak.get("status") == "NO_WEAK_LINK":
            st.metric("🔗 ASSEMBLY WEAK LINK", "N/A")
            st.success("Cavo a componente singolo: non viene creato un Weak Link separato. Si utilizza il carico di rottura applicabile del cavo.")
        elif weak:
            st.warning(f"Weak Link non ancora determinabile: {weak.get('diagnostic', 'completare i dati dei componenti')}" )
        ...
        if st.button("💾 Salva Certificato / Componenti", type="primary", use_container_width=True):
            ...
            if errors:
                ...
                record = {
                    ...
                    "manufacturer": manufacturer.strip(), "material_grade": material.strip(), "diameter_mm": float(diameter),
                    "length_m": float(first.get("length_m") or cdata.get("main_length_m") or 0.0), "ldbf_t": None,
                    "tail_tdbf_t": None, "tail_length_m": None,
                    "standard_basis": "DIN EN 10204 / DIN EN ISO 2307" if any(str(c.get("component_type")) == "TAIL" for c in comps) else "",
                    "issue_date": "", "strain": {}, "source_text": str(cdata.get("_source_text", cdata.get("raw_text", ""))),
                    "extraction_method": str(cdata.get("_extraction_method", "")), "review_status": "OPERATOR_VERIFIED",
                    "source_pdf_bytes": st.session_state.get("parsed_cert_pdf_bytes"), "source_pdf_filename": st.session_state.get("parsed_cert_filename", ""),
                ...
                legacy_capacity_t = weak_t if weak.get("status") == "VALID" and weak.get("has_weak_link") else main_governing_t
                legacy = {
                    "cert_id": cert_id.strip(), "manufacturer": manufacturer.strip(), "material": material.strip(), "diameter_mm": diameter,
                    "mbl_tons": legacy_capacity_t, "length_m": record["length_m"],
                    "weak_point": weak.get("weak_link_component_type", "Main Line") if weak.get("has_weak_link") else "Main Line",
                    "has_geolink": "YES" if geo else "NO", "geolink_mbl": (_applicable_tons(geo) or 0.0) if geo else 0.0,
                    "has_tail": "YES" if tail else "NO", "tail_material": tail.get("raw_material", "N/A") if tail else "N/A",
                ...
                if selected != "Nessuna (salva solo certificato)" and not lines_df.empty:
                    ...
                    if len(idx):
                        ...
                        lines_df.at[i, "diameter_mm"] = float(diameter)
                        lines_df.at[i, "main_mbl_tons"] = main_governing_t
                        lines_df.at[i, "weak_link_mbl_tons"] = legacy_capacity_t
                        lines_df.at[i, "weak_link_component"] = weak.get("weak_link_component_type", "MAIN LINE") if weak.get("has_weak_link") else "MAIN LINE"
                        lines_df.at[i, "weak_link_value_label"] = weak.get("weak_link_value_label", "")
                        lines_df.at[i, "mbl_tons"] = legacy_capacity_t
                ...
                st.success(f"Certificato {cert_id} salvato. " + (f"Weak link = {weak.get('weak_link_component_id')} @ {weak_t:.2f} t." if weak.get("has_weak_link") else "Capacità governante = main line."))
