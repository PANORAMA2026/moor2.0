...
def render_tab_certificate():
    ...
    with col_right:
        ...
        with st.form("form_save_certificate_and_line"):
            ...
            st.divider()
            lines_df=st.session_state.get("lines_inventory", pd.DataFrame()); line_options=["Nessuna (Salva solo Certificato)"]
            ...
            if btn_save:
                ...
                if errors:
                    ...
                    if selected_line_name != "Nessuna (Salva solo Certificato)" and not lines_df.empty:
                        ...
                        if not idx.empty:
                            i=idx[0]; lines_df.at[i,"cert_id"]=cert_id.strip(); lines_df.at[i,"material"]=material.strip(); lines_df.at[i,"diameter_mm"]=diameter_mm; lines_df.at[i,"mbl_tons"]=mbl_tons; lines_df.at[i,"length_m"]=length_m; lines_df.at[i,"winch_location"]=winch_location; lines_df.at[i,"tail_length_m"]=tail_len if has_tail else 0.0; lines_df.at[i,"tail_mbl_tons"]=tail_mbl if has_tail else 0.0; save_lines_inventory_to_db(lines_df); st.session_state.lines_inventory=load_lines_inventory_from_db()
    ...
    if records:
        ...
        st.markdown("### 📎 Documento originale"); cert_options=display["cert_id"].astype(str).tolist(); selected_cert=st.selectbox("Seleziona certificato",cert_options,key="certificate_document_selector"); document=get_certificate_pdf(selected_cert)
