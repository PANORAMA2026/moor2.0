...
class Wind:
    def coefficients(self, direction):
        return 1.0, 0.0, 0.0
...
def test_wind_and_current_are_combined_once():
    state = EnvironmentalState(
        timestamp_utc=datetime.now(timezone.utc),
        wind_speed_mps=10.0,
        wind_direction_from_deg_true=0.0,
        current_speed_mps=1.0,
        current_direction_to_deg_true=0.0,
...
def test_tidal_current_can_supply_total_current_when_no_total_is_present():
    state = EnvironmentalState(
        timestamp_utc=datetime.now(timezone.utc),
        tidal_current_u_mps=0.0,
        tidal_current_v_mps=0.5,
        provider="TEST",
        source_kind="FORECAST",
    )
    vessel = VesselHydroGeometry(100.0, 200.0, 50.0, 500.0, 300.0)
